...
namespace PVIMS.Web
{
    public partial class ReportPxOnStudy : MainPageBase
    {
        private int _fid = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                int tempi;
                if (int.TryParse(Request["fid"], out tempi))
                {
                    // User has clicked patient view
                    _fid = Convert.ToInt32(Request["fid"]);
                    ...
                    ddlCriteria.SelectedValue = Convert.ToString(Request["criteria"]);
        ...
        private void PopulateSummary()
        {
            ...
            var results = _reportService.GetPatientOnStudyItems(Convert.ToDateTime(searchfrom), Convert.ToDateTime(searchto), ddlCriteria.SelectedValue == "1" ? PatientOnStudyCriteria.HasEncounterinDateRange  : PatientOnStudyCriteria.PatientRegisteredinFacilityinDateRange);
            ...
            if (results.Count > 0)
            {
                foreach (PatientOnStudyList item in results)
                {
                    row = new TableRow();
                    ...
                    {
                        cell.Text = Math.Round(((decimal)(item.PatientWithEventCount * 100.00) / (decimal)item.PatientCount), 2).ToString() + "%";
                    }
                    else
                    {
                        cell.Text = "N/A";
                    ...
                    var action = String.Format(@"<div class=""btn-group""><a class=""btn btn-default"" href=""ReportPxOnStudy.aspx?fid={0}&searchfrom={1}&searchto={2}&criteria={3}"">View Patients</a></div><!-- /btn-group -->", item.FacilityId.ToString(), searchfrom, searchto, ddlCriteria.SelectedValue);
                    cell = new TableCell();
                    cell.Text = action;
                    row.Cells.Add(cell);
        ...
        private void PopulatePatient()
        {
            ...
            var results = _reportService.GetPatientListOnStudyItems(Convert.ToDateTime(searchfrom), Convert.ToDateTime(searchto), ddlCriteria.SelectedValue == "1" ? PatientOnStudyCriteria.HasEncounterinDateRange : PatientOnStudyCriteria.PatientRegisteredinFacilityinDateRange, _fid);
        ...
        protected void btnExportPdf_Click(object sender, EventArgs e)
        {
            ...
            // Write filter
            linePosition += 24;
            gfx.DrawString("Criteria : " + ddlCriteria.SelectedItem.Text, fontr, XBrushes.Black, new XRect(columnPosition, linePosition, page.Width.Point, 20), XStringFormats.TopLeft);
            linePosition += 24;
            ...
            foreach (TableRow row in dt_basic.Rows)
            {
                ...
                if (linePosition >= 480)
                {
                    ...
                    gfx.DrawString("Patient On Treatment Report (Page " + pageCount.ToString() + ")", fontb, XBrushes.Black, new XRect(columnPosition, linePosition, page.Width.Point, 20), XStringFormats.TopLeft);
