...
namespace PVIMS.Web
{
    public partial class ReportCausality : MainPageBase
    {
        ...
        protected void btnExportXls_Click(object sender, EventArgs e)
        {
            ...
            var ws = pck.Workbook.Worksheets.Add("Causality Report");
            ...
            Response.AddHeader("content-disposition", String.Format("attachment;filename=ReportCausality_{0}.xlsx", DateTime.Now.ToString("yyyyMMddhhmm")));
            Response.Charset = "";
            this.EnableViewState = false;
    ...
    {
        public static void SetColumn(this Table table, int index, string name, Unit width)
        {
        ...
        public static void ShowColumn(this Table table, int index)
        {
            foreach (TableRow row in table.Rows)
            {
                if (row.Cells.Count - 1 >= index)
                {
                    row.Cells[index].Visible = true;
                }
            }
        }

        public static void HideColumnByRange(this Table table, int fromIndex, int toIndex)
        {
            foreach (TableRow row in table.Rows)
            {
